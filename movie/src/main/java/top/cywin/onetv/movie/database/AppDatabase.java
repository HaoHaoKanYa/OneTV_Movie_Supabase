package top.cywin.onetv.movie.database;
...
import top.cywin.onetv.movie.bean.Backup;
...
import top.cywin.onetv.movie.database.dao.ConfigDao;
import top.cywin.onetv.movie.database.dao.DeviceDao;
import top.cywin.onetv.movie.database.dao.HistoryDao;
import top.cywin.onetv.movie.database.dao.KeepDao;
import top.cywin.onetv.movie.database.dao.LiveDao;
import top.cywin.onetv.movie.database.dao.SiteDao;
import top.cywin.onetv.movie.database.dao.TrackDao;
...
public abstract class AppDatabase extends RoomDatabase {
    ...
    public static void backup() {
        backup(new top.cywin.onetv.movie.impl.Callback());
    }

    public static void backup(top.cywin.onetv.movie.impl.Callback callback) {
        App.execute(() -> {
            File file = new File(Path.movie(), "movie-" + new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date()) + ".bk");
    ...
    public static void restore(File file, top.cywin.onetv.movie.impl.Callback callback) {
        App.execute(() -> {
    ...
    private static void cleanOld() {
        ...
        File[] files = Path.movie().listFiles();
        if (files == null) files = new File[0];
        for (File file : files) if (file.getName().startsWith("movie") && file.getName().endsWith(".bk.gz")) items.add(file);
