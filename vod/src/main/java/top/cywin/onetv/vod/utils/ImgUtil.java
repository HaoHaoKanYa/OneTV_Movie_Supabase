...
public class ImgUtil {
    ...
    public static void load(String text, String url, ImageView view, ImageView.ScaleType scaleType, boolean rect) {
        ...
        if (!TextUtils.isEmpty(url)) Glide.with(App.get()).asBitmap().load(getUrl(url)).placeholder(R.drawable.vod_ic_img_loading).skipMemoryCache(true).dontAnimate().sizeMultiplier(Setting.getThumbnail()).signature(getSignature(url)).listener(getListener(view, scaleType)).into(view);
        else if (!text.isEmpty()) view.setImageDrawable(getTextDrawable(text.substring(0, 1), rect));
        else view.setImageResource(R.drawable.vod_ic_img_error);
    }

    public static void loadVod(String text, String url, ImageView view) {
        view.setScaleType(ImageView.ScaleType.CENTER);
        if (!TextUtils.isEmpty(url)) Glide.with(App.get()).asBitmap().load(getUrl(url)).placeholder(R.drawable.vod_ic_img_loading).listener(getListener(view)).into(view);
    ...
    public static void loadLive(String url, ImageView view) {
        ...
        if (TextUtils.isEmpty(url)) view.setImageResource(R.drawable.vod_ic_img_empty);
        else Glide.with(App.get()).asBitmap().load(getUrl(url)).error(R.drawable.vod_ic_img_empty).skipMemoryCache(true).dontAnimate().signature(getSignature(url)).into(view);
    }
    ...
    private static RequestListener<Bitmap> getListener(ImageView view, ImageView.ScaleType scaleType) {
        return new RequestListener<>() {
            ...
            public boolean onLoadFailed(@Nullable GlideException e, Object model, @NonNull Target<Bitmap> target, boolean isFirstResource) {
                view.setImageResource(R.drawable.vod_ic_img_error);
                view.setScaleType(scaleType);
                return true;
            }
